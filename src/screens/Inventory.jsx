              ...
              {(FILTER_CONFIG[activeTab] ?? []).map((f) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase transition-all whitespace-nowrap ${state === 'yes' ?
                            ...
                            return (
                              <div className="grid gap-px border-t border-white/5" style={{ gridTemplateColumns: `repeat(${Math.min(set.parts.length, 6)}, 1fr)` }}>
                                {set.parts.map((part, pi) => {
                                  ...
                                  const partCell =
                                    <div className={`flex flex-col items-center justify-center gap-1.5 p-3 h-full ${isWarframeComponent ? (hasBlueprint ? 'bg-yellow-400/10' : met ? 'bg-green-400/10' : 'bg-black/20') : (met ? 'bg-green-500/10' : 'bg-black/20')} relative`}>
                                      {part.need > 1 &&
